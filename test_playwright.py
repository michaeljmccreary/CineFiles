...
class CineFilesPlaywrightTests:
    ...
    async def ensure_test_user_exists(self):
        ...
        try:
            ...
            await self.page.click('form:has(input[name="username"]) button[type="submit"]', timeout=10000)
            await self.page.wait_for_load_state("networkidle", timeout=30000)
    ...
    async def test_03_user_registration(self):
        ...
        try:
            ...
            await self.page.click('form:has(input[name="username"]) button[type="submit"]')
            await self.page.wait_for_load_state("networkidle")
    ...
    async def test_04_user_login(self):
        ...
        try:
            ...
            await self.page.click('form:has(input[name="password"]) button[type="submit"]')
            await self.page.wait_for_load_state("networkidle")
    ...
    async def test_06_edit_profile(self):
        ...
        try:
            ...
            await self.page.click('form:has(textarea[name="bio"]) button[type="submit"]')
            await self.page.wait_for_load_state("networkidle")
    ...
    async def test_12_sql_injection_login(self):
        ...
        try:
            ...
            # Check if we were redirected to profile (successful SQL injection)
            # or stayed on login page (SQL injection blocked)
            current_url = self.page.url.lower()
            
            if "profile" in current_url:
                # If we got to the profile page without valid credentials, it's vulnerable
                await self.take_screenshot("12_sql_injection_success")
                self.log_result(
                    "SQL Injection Test",
                    "VULNERABILITY",
                    "SQL injection successful! Login bypassed with malicious payload.",
                )
            elif "login" in current_url:
                # If we're still on the login page, the injection was blocked
                await self.take_screenshot("12_sql_injection_blocked")
                self.log_result(
                    "SQL Injection Test",
                    "PASS",
                    "SQL injection blocked. Login form appears to be secure.",
                )
            else:
                # Unexpected redirect
                await self.take_screenshot("12_sql_injection_unexpected")
                self.log_result(
                    "SQL Injection Test",
                    "ERROR",
